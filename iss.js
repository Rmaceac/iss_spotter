...
const nextISSTimesForMyLocation = (callback) => {
  ...
  request(getIpURL, (error, response, body) => {
    ...
    // callback(null, ipAddress); <----- beyond errors, callback only needed for final result

    // GET LAT/LONG FUNCTION

    const geoURL = `https://freegeoip.app/json/${ipAddress}`;
    ...
    request(geoURL, (error, response, body) => {
      ...
      // callback(null, latLong); <----- beyond errors, callback only needed for final result

      // GET FLYOVERS FUNCTION

      const ISSURL = `https://iss-pass.herokuapp.com/json/?lat=${latLong.latitude}&lon=${latLong.longitude}`;
